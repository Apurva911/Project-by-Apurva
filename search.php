<?php  include ('header.php');
   ...
   if(empty($keyword)){
	...
	$ofset=($page-1)*$limit;
	//---------
    $sql="SELECT * FROM blog LEFT JOIN categories ON blog.category=categories.cat_id LEFT JOIN user ON blog.author_id=user.user_id WHERE blog_title like '%$keyword%' OR blog_body like '%$keyword%' ORDER BY blog.publish_date DESC  limit $ofset,$limit";
			...
			<div class="card shadow">
				<div class="card-body d-flex blog_flex">
					<div class="flex-part1">
						<a href="single_post.php?id=<?=$result['blog_id']?>"><?php $img=$result['blog_image'] ?><img src="admin/upload/<?=$img?>"> </a>
					</div>
					<div class="flex-grow-1 flex-part2">
						  <a href="single_post.php?id=<?=$result['blog_id']?>" id="title"></a>
						  <?= ucfirst($result['blog_title'])?>
						<p>
						  <a href="single_post.php?id=<?=$result['blog_id']?>" id="body">
						  <?= strip_tags(substr(($result['blog_body']),0,50))."..."?>
						  </a> <span><br>
