				...
				<div class="card-body d-flex right-section">
					<div id="categories">
						...
						<ul>
							...
							<li><a href="cat.php?id=<?= $cats['cat_id'] ?>" class="text-success fw-bold"><?= $cats['cat_name'] ?></a></li>
							<?php } ?>
						</ul>
					</div>
				    <div id="posts">
						...
						<ul>
							<?php while($posts=mysqli_fetch_assoc($query2)) {?>
								<li><a href="single_post.php?id=<?= $posts['blog_id']?>" class="text-success fw-bold"><?= $posts['blog_title']?></a>
								</li>
								<?php }?>
						</ul>
					</div>
